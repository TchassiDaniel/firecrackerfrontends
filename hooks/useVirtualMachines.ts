...
import { VirtualMachine, SystemImage, VMmodels, VMMetrics, VMStatusHistory, VmCreate } from '@/types/virtualMachine';
...
export const useVirtualMachines = () => {
  ...
  const fetchVirtualMachineById = useCallback(async (id: number): Promise<VirtualMachine> => {
    try {
      setIsLoading(true);
      setError(null);
 ...
 //creer une vm
  const createVirtualMachine = useCallback(async (data: VmCreate) => {
    try {
      console.log('URL de création:', API_ENDPOINTS.VIRTUAL_MACHINES.endpoints.CREATE);
      ...
      if (err.response) {
        ...
        setError(`Erreur serveur: ${err.response.data.message || err.response.statusText}`);
      } else if (err.request) {
  ...
  const updateVirtualMachineStatus = useCallback(async (vmId: number, action: "start" | "stop" | "pause") => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      setVirtualMachines(prevVMs => prevVMs.map(vm => 
        vm.id === vmId ? { ...vm, status: response.data.status } : vm
      ));
      
      return response.data;
    } catch (err) {
      setError(`Erreur lors de la mise à jour du statut de la machine virtuelle`);
      console.error(err);
      throw err;
    } finally {
      setIsLoading(false);
  ...
  const fetchSystemImages = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await systemImagesClient.get(API_ENDPOINTS.SYSTEM_IMAGES.endpoints.LIST);
  ...
  const fetchVMModels = useCallback(async () => {
    try {
      ...
      setIsLoading(true);
      setError(null);
      const response = await vmClient.get(API_ENDPOINTS.VM_MODELS.endpoints.LIST);
  ...
  const fetchVMMetrics = useCallback(async (id: number): Promise<VMMetrics> => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await vmClient.get(`${API_ENDPOINTS.VIRTUAL_MACHINES.endpoints.GET(id)}/metrics`);
      return response.data;
    } catch (err) {
  ...
  const fetchVMStatusHistory = useCallback(async (id: number): Promise<VMStatusHistory[]> => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await vmClient.get(`${API_ENDPOINTS.VIRTUAL_MACHINES.endpoints.GET(id)}/status-history`);
      return response.data;
    } catch (err) {
